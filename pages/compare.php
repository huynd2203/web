...
// Truy vấn để lấy danh sách sản phẩm các danh mục
...
if (isset($_SESSION['compare']) && !empty($_SESSION['compare'])) {
    ...
    if (!empty($compareIds)) {
        $sql = "SELECT products.ID, products.Name, products.Price, products.Discount, products.Description, products_images.ImageURL
                FROM products
                JOIN products_images ON products.ID = products_images.productID
                WHERE products.ID IN ($compareIds)";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td><img src='{$row['ImageURL']}' alt='phone Image'></td>";
                echo "<td>{$row['Name']}</td>";
                echo "<td>{$row['Description']}</td>";
                $final_price = $row['Price'];
                echo "<td style='color: #dc3545; font-weight: 700;'>" . number_format($final_price, 0, '.', ',') . "</td>";
                echo "<td><a id='seemore-compare' href='product_detail.php?ProductID={$row['ID']}'>Xem chi tiết</a></td>";
                echo "<td><form action='remove_compare.php' method='POST'><input type='hidden' name='productID' value='" . $row['ID'] . "'><button type='submit' class='remove-button' style='background-color: transparent; border: none; cursor: pointer;'><i class='fa-solid fa-xmark'></i></button></form></td>";
                echo "</tr>";
            }
            echo "</table>";
        }
         else {
            echo "<h3 style='font-size: 29px; font-weight: 400; color: #333e48; text-align: center;'>Không có món ăn nào được thêm vào bảng so sánh</h3>";
            echo "<a href='index_home.php'>Trở về trang chủ</a>";
        }
    } else {
        echo "<h3 style='font-size: 29px; font-weight: 400; color: #333e48; text-align: center; margin: 30px 0px;'>Không có món ăn nào được thêm vào bảng so sánh</h3>";
